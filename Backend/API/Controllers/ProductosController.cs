...
namespace API.Controllers
{
 ...
 {
     var folder = _uploadsDir.FullName;
...
using(var stream = new FileStream(rutaFisica, FileMode.Create, FileAccess.Write, FileShare.None))
    ...
    try
 {
   ...
   var ext = Path.GetExtension(form.Imagen.FileName).ToLowerInvariant();
      ...
      if(form.Imagen.Length > 5*1024*1024) return BadRequest("Imagen demasiado grande (máx 5MB)");
...
Activo = form.Activo,
 ...
 if(data.StartsWith("data:"))
  ...
  var headerSep = data.IndexOf(",");
      ...
      var header = data.Substring(0, headerSep);
 ...
 if(parts.Length==2) mime = parts[1];
    ...
    try { bytes = Convert.FromBase64String(base64); } 
      catch { return BadRequest("Base64 inválido"); }
