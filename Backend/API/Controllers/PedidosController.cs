...
namespace API.Controllers
{
 ...
 try
 {
 // Obtener GUID del usuario autenticado (sub o NameIdentifier)
 var rawId = User.FindFirstValue(ClaimTypes.NameIdentifier) ?? User.FindFirstValue(JwtRegisteredClaimNames.Sub);
 if (Guid.TryParse(rawId, out var userGuid))
 {
 dto.UsuarioId = userGuid.ToString();
 }
 ...
 {
 var rawId = User.FindFirstValue(ClaimTypes.NameIdentifier) ?? User.FindFirstValue(JwtRegisteredClaimNames.Sub);
 if (!Guid.TryParse(rawId, out var userGuid)) return Unauthorized(new { mensaje = "Token sin identificador de usuario" });
 var pedidos = await _listar.EjecutarAsync();
 var propios = pedidos.Where(p => p.UsuarioId != null && p.UsuarioId == userGuid);
 return Ok(propios);
 }
 ...
 var pedido = await _pedidoRepo.ObtenerPorIdAsync(id);
 if(pedido==null) return NotFound();
 // Con el nuevo flujo (PENDIENTE->EN_PREPARACION->LISTO) simplemente marcamos historial y no cambiamos estado
